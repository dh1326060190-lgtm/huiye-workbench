...
const Planner = {
  ...
  createLinkedTasks(plan) {
    ...
    if (isHouse) {
      shootList = [
        '镜头1（开场3秒）：入户门第一视角推门进入，配字幕"收房千万别急着签字！"',
        '镜头2：入户门检查——手持特写门框缝隙、门锁、门吸，只拍手部和门',
        '镜头3：墙面空鼓——手持空鼓锤敲击墙面，特写锤子+听声音（只拍手和墙）',
        '镜头4：地面平整——手机水平仪放地上特写数值，俯拍地面',
        '镜头5：窗户密封——纸条夹窗缝特写，手推拉窗户拍滑轨',
        '镜头6：水电检查——手持拍开关插座、水龙头出水、地漏排水（只拍手和物件）',
        '镜头7（结尾）：俯拍验房清单表格/手机备忘录，配字幕"完整清单评论区领"',
      ];
    } else if (isRenovate) {
      shootList = [
        '镜头1（开场3秒）：施工现场全景扫视，配字幕"装修第X天：XXX"',
        '镜头2：施工前状态记录——手持拍毛坯/拆除后全貌',
        '镜头3：施工过程特写——手持拍工人操作、材料进场（不拍人脸）',
        '镜头4：关键节点——手持拍施工细节，配画外音讲解（不露脸）',
        '镜头5：验收细节——俯拍卷尺/水平仪检测过程，只拍手和工具',
        '镜头6（结尾）：手持拍完工效果，配字幕"明天继续XXX，关注不迷路"',
      ];
    } else if (isVisual) {
      shootList = [
        '镜头1（开场3秒）：成品全景缓慢扫视，配字幕"收房后装成了这样"',
        '镜头2：设计效果图→实景转场对比（固定机位拍实景，剪辑做对比）',
        '镜头3：色彩搭配——手持逐一拍墙面/家具/软装，配画外音讲解',
        '镜头4：空间利用——手持开收纳柜、展示多功能区域（只拍柜子和手）',
        '镜头5：细节特写——固定拍灯具/把手/装饰画等质感镜头',
        '镜头6（结尾）：全景收尾，配字幕"这样的风格你们喜欢吗？评论区告诉我"',
      ];
    } else if (isFurniture) {
      shootList = [
        '镜头1（开场3秒）：所有好物摆一排俯拍全景，配字幕"收房后这些钱绝对不能省"',
        '镜头2-6：每件好物单独展示——手持拍开箱→使用场景→特写细节→价格标签',
        '镜头7：使用前后对比（固定机位拍同一个场景，有/没有这件物品）',
        '镜头8（结尾）：俯拍所有物品，配字幕"你最想买哪件？评论区告诉我"',
      ];
    } else {
      shootList = [
        '镜头1（开场3秒）：最吸引人的画面/冲突场景，配悬念字幕（不露脸）',
        '镜头2-5：主体内容分点拍摄，手持/固定机位，配画外音讲解',
        '镜头6（结尾）：俯拍总结要点，配字幕互动引导"你们觉得呢？"',
      ];
    }
    ...
    if (isHouse) {
      editSteps = [
        ...
        '第6步：片尾——关注引导字幕动画+"完整验房清单评论区领"文字停留3秒',
      ];
    } else if (isRenovate) {
      editSteps = [
